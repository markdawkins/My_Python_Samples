...
def main():
    ...
    with open(output_file, "w") as f:
        ...
        if comparison.empty:
            ...
            # Restructure comparison DataFrame into readable rows
            differences = []
            for row_idx in comparison.index:
                for col in comparison.columns.levels[0]:
                    ...
                    if pd.notna(val1) or pd.notna(val2):  # only log actual differences
                        differences.append(
                            f"Row {row_idx}, Column '{col}': File1 = {val1}, File2 = {val2}"
                        )
