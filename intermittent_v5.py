...
ERROR_LOG = "ping_errors.log"
PING_COUNT = 5
PING_INTERVAL = 180       # 3 minutes in seconds
RUN_DURATION = 1800       # 30 minutes in seconds
...
def ping_target(target):
    ...
    try:
        ...
        if subprocess.os.name == "nt":  # Windows uses -n
        ...
        # Return the error message and an "Error" status
        return str(e), "Error"
...
def log_error(target, error_msg):
    """Logs errors to a separate file."""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(ERROR_LOG, mode='a', encoding='utf-8') as file:
        file.write(f"[{timestamp}] Target: {target} | Error: {error_msg}\n")
...
def main():
    ...
    print("Starting intermittent ping... (will stop after 30 minutes)")
    ...
    while (time.time() - start_time) < RUN_DURATION:
        for target in targets:
            ...
            if status == "Error":
                log_error(target, output)
            else:
                write_output(target, status, output)
        print("Ping round completed")
        time.sleep(PING_INTERVAL)

    print("Script completed after 30 minutes. Exiting...")
